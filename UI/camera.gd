...
func _ready():
	if multiplayer.is_server():
		connect("area_selected", Callable(get_tree().get_root().get_node("World/1"), "_on_area_selected"))
	else:
		connect("area_selected", Callable(get_tree().get_root().get_node("World/0"), "_on_area_selected"))
func _process(delta):
